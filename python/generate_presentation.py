"""
Executive Presentation Generator Module

This module automates the construction of high-level PowerPoint decks (.pptx) 
summarizing the analytical findings, with a specific focus on churn risk 
identification and long-term customer value.
"""
...
# Establish database connection and extract enriched results.
engine = create_engine(CONNECTION_STRING)
try:
    df = pd.read_sql("SELECT * FROM RFM_Results_Output", engine)
except Exception as e:
    print(f"Error extracting analytical results from database: {e}")
    print("Ensure the main ETL pipeline has been successfully executed.")
    sys.exit(1)
finally:
    engine.dispose()
...
# Create and save the executive presentation.
prs = Presentation()
add_title_slide(prs)
...
prs.save(PPT_PATH)
print("-" * 60)
print(f"Execution Successful: Executive presentation generated at: {PPT_PATH}")
print("-" * 60)
